...
class ScrumTimer: ObservableObject {
    ...
    private func update(secondsElapsed: Int) {
        ...
        if secondsElapsedForSpeaker >= secondsPerSpeaker {
            changeToSpeaker(at: speakerIndex + 1)
            speakerChangedAction?()   /// The ScrumTimer class periodically updates the state of a meeting. The timer calls the speakerChangedAction closure when the speaker changes and shares other important state changes through @Published properties.
        }
    }
